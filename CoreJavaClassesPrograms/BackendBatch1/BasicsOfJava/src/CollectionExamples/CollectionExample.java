...
//		System.out.println("Pool first : "+set.pollFirst());
	...
	public static void streamExample() {
		...
		Optional value = list1.stream().filter((item) -> {
			if(((int)item)%2 == 0) {
				return true;
			}
			return false;
		 ...
		 * [1, 2, 3, 4, 5, 6]
		 * 
		 *  reduce - will reduce the list to a single value
		 *  1. accu		-	
		 *  	1st iteration - 0 index
		 *  	2nd iteration - returning value
		 *  2. current
		 *  	1st iteration - 1 index value
		 *  
		 *  1st iteration
		 *  	acc - 1, curr - 2
		 *  	
